...
interface WalletBalanceHook {
  ...
  fetchSingleBalance: (walletAddress: string, tokenSymbol: string) => Promise<TokenBalance>;
...
export const useWalletBalance = (): WalletBalanceHook => {
  ...
  const updateTokenData = useCallback((symbol: string, balance: string, balanceUsd: string) => {
  ...
  const fetchAllBalances = useCallback(async (walletAddress: string): Promise<void> => {
    ...
    try {
      ...
      const wattsInUsd = (parseFloat(formattedWattsBalance) * rates.USDC).toFixed(2);
      ...
      const ethInUsd = (parseFloat(formattedEthBalance) * rates.ETH).toFixed(2);
  ...
  const fetchSingleBalance = useCallback(async (walletAddress: string, tokenSymbol: string): Promise<TokenBalance> => {
    if (!walletAddress) {
    ...
    try {
      ...
      switch (normalizedTokenSymbol) {
        case 'WATT': {
          ...
          const balanceInUsd = (parseFloat(formattedBalance) * rates.USDC).toFixed(2);
          tokenBalance = {balance: formattedBalance, balanceUsd: balanceInUsd};
          break;
        }
        ...
        case 'ETH': {
          ...
          const balanceInUsd = (parseFloat(formattedBalance) * rates.ETH).toFixed(2);
          tokenBalance = {balance: formattedBalance, balanceUsd: balanceInUsd};
          break;
        }
        ...
        default: {
          ...
          const tokenInfo = tokenMapping[normalizedTokenSymbol as keyof typeof tokenMapping];

          if (!tokenInfo) {
            throw new Error(`Unknown token symbol: ${normalizedTokenSymbol}`);
          }

          const provider = tokenInfo.network === CUSTOM_NETWORK ? denergyProvider : sepoliaProvider;
          const contractAddress = TOKEN_CONTRACTS[tokenInfo.network][tokenInfo.token];

          const contract = new ethers.Contract(contractAddress, ERC20_ABI, provider);
          const balance = await contract.balanceOf(walletAddress);
          const formattedBalance = ethers.formatUnits(balance, 6);
          const balanceInUsd = (parseFloat(formattedBalance) * rates[tokenInfo.rateKey]).toFixed(2);
      ...
      // Update the state
      updateTokenData(normalizedTokenSymbol, tokenBalance.balance, tokenBalance.balanceUsd);
  ...
  const getBalance = useCallback((tokenSymbol: string): TokenBalance => {
