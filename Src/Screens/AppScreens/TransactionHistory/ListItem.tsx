...
interface ListItemProps {
  ...
  name: string;
  setSelectedItems: (item: TransactionItem) => void;
}
...
const ListItem: React.FC<ListItemProps> = ({item, name, setSelectedItems}) => {
  ...
  const color = item.type === 'Deposit' ? '#007E32' : '#DB0A0A';
      ...
      <View style={styles.leftContainer}>
        <View style={styles.contentRow}>
          ...
          <View style={styles.textContainer}>
            <Text style={[styles.transationType, styles.capitalize]}>
              {item.type}
            </Text>
      ...
      <View style={styles.rightContainer}>
        <Text style={[styles.amount, {color: color}]}>
          ...
          {item.amount >= 0
            ...
            : parseFloat(item.tokenAmount.toString())}{' '}
          {item.coinCode}
        </Text>
